...
`TokenRemote` instances are registered with their home contract via a Interchain Messaging message by calling the `registerWithHome` function  on the remote contract.
